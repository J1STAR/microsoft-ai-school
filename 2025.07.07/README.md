...
-   **서비스 클래스 모듈화**: 얼굴 탐지(`_Face`)와 객체 탐지(`_Yolo`) 로직을 각각의 헬퍼 클래스로 캡슐화하고, 이를 총괄하는 `OpenCVService`를 설계하여 코드의 구조성과 재사용성을 높입니다.
...
이날의 프로젝트는 사용자가 자신의 웹캠이나 이미지 파일을 통해 **실시간 얼굴 탐지**와 **YOLO 객체 탐지**를 직접 체험할 수 있는 대화형 웹 애플리케이션입니다.
...
2.  **YOLO 객체 탐지 (YOLO Detection)**: 널리 사용되는 객체 탐지 모델인 YOLOv3를 사용하여 웹캠 영상 속의 다양한 사물(사람, 컵, 책 등)을 실시간으로 인식합니다. 탐지된 객체는 한국어 레이블과 신뢰도 점수가 포함된 바운딩 박스로 표시됩니다.
...
| 파일명 | 설명 |
| :--- | :--- |
| `main.py` | Gradio를 사용하여 **얼굴 탐지**와 **YOLO 객체 탐지** 탭을 가진 메인 애플리케이션 UI를 구성하고, 각 UI 컴포넌트의 이벤트를 `OpenCVService` 함수와 연결하는 역할을 합니다. |
| `services/opencv_service.py` | `_Face`와 `_Yolo`라는 두 개의 내부 헬퍼 클래스를 통해 얼굴 탐지 및 객체 탐지의 핵심 로직을 수행하는 `OpenCVService` 클래스를 정의합니다. |
| `models/` | `yolov3`와 `yolov8` 모델 파일들(`*.weights`, `*.cfg`, `*.names`)이 저장된 디렉터리입니다. |
...
`OpenCVService`는 내부적으로 **얼굴 탐지(`_Face`)**와 **객체 탐지(`_Yolo`)**라는 두 가지 역할을 명확히 분리된 헬퍼 클래스로 관리합니다.

-   **`OpenCVService`**: 메인 클래스로, `_Face`와 `_Yolo` 인스턴스를 생성하고 관리합니다. `@property` 데코레이터를 사용하여 외부(main.py)에서는 `opencv_service.face`나 `opencv_service.yolo`와 같이 각 기능에 명확하고 직관적으로 접근할 수 있도록 합니다.
-   **`_Face`**: Haar Cascade 모델을 로드하고, `detectMultiScale` 함수를 사용하여 얼굴을 탐지하는 모든 로직을 캡슐화합니다.
-   **`_Yolo`**: YOLOv3 네트워크를 로드하고, 입력 이미지를 `blob`으로 변환하여 네트워크에 전달한 뒤, 결과에서 바운딩 박스를 추출하고 비최대 억제(NMS)를 적용하는 복잡한 과정을 담당합니다.

이러한 구조는 코드의 가독성을 높이고, 향후 새로운 탐지 모델(예: `_YoloV8`)을 추가하는 등의 확장을 용이하게 만듭니다.
...
class OpenCVService:
    def __init__(self) -> None:
        ...
        self._yolo = _Yolo(
            ...
            names_path=os.path.join(..., "coco_korean.names"),
        )
...
`main.py`에서는 Gradio의 `Blocks` API를 사용하여 사용자 인터페이스를 구축합니다. 특히, `stream()`과 `change()` 이벤트 핸들러는 이 애플리케이션의 핵심적인 상호작용을 구현합니다.

-   **`stream(fn, inputs, outputs)`**: 웹캠(`Image(sources="webcam", streaming=True)`) 컴포넌트와 함께 사용됩니다. 웹캠에서 새로운 프레임이 들어올 때마다 `fn`으로 지정된 함수(예: `opencv_service.face.detect`)가 자동으로 호출되고, 그 결과가 `outputs` 컴포넌트에 실시간으로 표시됩니다.
...
# 웹캠 스트림이 들어올 때마다 실시간으로 얼굴 탐지를 수행합니다.
input_webcam.stream(
...
`_Yolo.detect` 메서드는 객체 탐지의 복잡한 파이프라인을 수행합니다.
...
-   **코드 구조의 중요성**: 기능이 복잡해질수록, `OpenCVService`처럼 역할을 명확히 나누어 클래스로 캡슐화하는 설계가 코드의 유지보수성과 확장성을 얼마나 향상시키는지를 체감할 수 있었습니다.
